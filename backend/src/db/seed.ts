...
export async function seedInitialData() {
  const existingDisc = sqlite.prepare(
    'SELECT id FROM disciplinas WHERE nome = ?'
  ).get('IHC: Interação Humano Computador');
  ...
  const disciplinaId = queries.criarDisciplina('IHC: Interação Humano Computador');
  
  const temaId = queries.criarTema('UX/UI', disciplinaId);
  ...
  for (const carta of gameCards) {
    queries.criarCarta(carta.nome, temaId, carta.dicas);
  }
  
  console.log('Seed completo executado - 10 cartas criadas');
}